...
/**
 * AI Playground — a real, honest model running entirely in your browser.
 * A simplified logistic-regression churn classifier (the kind behind my Telco
 * Churn project): adjust the inputs and watch the probability — and each
 * feature's contribution to it — update live. No server, no API key, just math.
 */
  ...
  return (
    <section id="playground" className="scroll-mt-24 py-24">
      <SectionHeading
        ...
        kicker="live_model"
        title="AI Playground"
        subtitle="A real model, running in your browser — no server, no API key."
      />

      <Reveal className="glass mx-auto max-w-4xl overflow-hidden rounded-3xl border border-white/10">
        ...
        <div className="grid gap-px sm:grid-cols-[1.1fr_1fr]">
          {/* controls */}
          <div className="space-y-5 bg-void-900/30 p-6 sm:p-7">
            <Slider
              label="Tenure"
              value={tenure}
              min={0}
          ...
          {/* result */}
          <div className="flex flex-col bg-void-850/40 p-6 sm:p-7">
            ...
            <div className="mt-2 flex items-end gap-3">
              ...
              <span
                className="mb-1.5 rounded-full px-2.5 py-0.5 text-[0.6rem] font-semibold"
            ...
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-void-700">
              <div
                ...
                style={{ width: `${pct}%`, background: riskColor, boxShadow: `0 0 12px ${riskColor}` }}
              />
            </div>

            {/* feature contributions */}
            <p className="mono-label mt-6 text-[0.55rem] text-neutral-500">
              what's driving it
            </p>
            ...
            <p className="mt-auto pt-5 text-[0.7rem] leading-relaxed text-neutral-500">
              ← drives churn down &nbsp;·&nbsp; drives it up →. The model weighs
              each signal, sums them, and squashes to a probability — exactly
              what ran over 500k customers in the real project.
            </p>
          </div>
        </div>
      </Reveal>
