    ...
    <div className="stage">
            ...
            <svg className="proj-chart" viewBox="0 0 320 120" preserveAspectRatio="none">
              <defs>
                <linearGradient id="pg" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="rgba(255,122,47,0.35)" />
              ...
              <polyline className="proj-path" points="0,86 46,72 92,78 138,52 184,58 230,32 276,40 320,18" fill="none" stroke="#ff7a2f" strokeWidth="2.5" />
            </svg>
          ...
          <div className="edu">
            <div className="edu-cap">
              <svg viewBox="0 0 64 64" width="58" height="58" aria-hidden="true">
                <path d="M32 14 L60 26 L32 38 L4 26 Z" fill="#ffb061" />
                <path d="M16 31 L16 44 Q32 52 48 44 L48 31" fill="none" stroke="#ff7a2f" strokeWidth="3" />
                <line x1="60" y1="26" x2="60" y2="40" stroke="#ffb061" strokeWidth="2.5" />
                <circle cx="60" cy="42" r="3" fill="#ff7a2f" />
              </svg>
            </div>
      ...
      <style jsx>{`
        .stage { position: relative; height: 232px; border-radius: 0.9rem; overflow: hidden;
          background: radial-gradient(120% 120% at 50% 0%, #14101a, #0a0810 70%); border: 1px solid rgba(255,255,255,0.06); }
        .stage-bg { position: absolute; inset: 0; opacity: 0.7; }
        .stage-grad { position: absolute; inset: 0; pointer-events: none;
          background: radial-gradient(90% 80% at 50% 30%, transparent 40%, rgba(8,7,10,0.7) 100%); }
        ...
        .core { width: 18px; height: 18px; margin: 0 auto 0.7rem; border-radius: 9999px;
          background: radial-gradient(circle, #ffd9a8, #ff7a2f); box-shadow: 0 0 24px rgba(255,122,47,0.8);
        ...
        .ring { position: absolute; left: 50%; top: 9px; border-radius: 9999px; border: 1px solid rgba(255,122,47,0.5);
        ...
        .hello-kicker { font-size: 0.55rem; letter-spacing: 0.3em; color: #ffb061; animation: rise 0.5s both 0.2s; }
        .hello-name { font-size: 1.5rem; font-weight: 800; color: #fff; margin-top: 0.2rem; animation: rise 0.6s both 0.35s; }
        .hello-sub { font-size: 0.72rem; color: rgba(244,239,233,0.6); letter-spacing: 0.12em; margin-top: 0.25rem; animation: rise 0.6s both 0.5s; }
        ...
        .exp-line { position: absolute; top: 12px; left: 8%; right: 8%; height: 2px; transform-origin: left;
          background: linear-gradient(90deg, rgba(255,122,47,0.2), #ff7a2f, rgba(124,120,240,0.6)); animation: draw 0.8s ease both; }
        ...
        .exp-dot { display: block; width: 11px; height: 11px; border-radius: 9999px; margin: 7px auto 0.6rem;
          background: #ff7a2f; box-shadow: 0 0 12px rgba(255,122,47,0.9); }
        .exp-name { font-size: 0.95rem; font-weight: 800; color: #fff; }
        .exp-role { font-size: 0.62rem; color: rgba(244,239,233,0.65); margin-top: 0.1rem; }
        .exp-yr { font-size: 0.6rem; color: #ffb061; margin-top: 0.15rem; font-variant-numeric: tabular-nums; }

        /* Projects */
        .proj { width: 100%; max-width: 420px; }
        .proj-chart { width: 100%; height: 96px; }
        .proj-path { stroke-dasharray: 600; stroke-dashoffset: 600; animation: dash 1.3s ease forwards; filter: drop-shadow(0 0 6px rgba(255,122,47,0.6)); }
        ...
        .proj-chip { font-size: 0.64rem; color: #f4efe9; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,122,47,0.3);
        ...
        .imp-label { font-size: 0.56rem; color: rgba(244,239,233,0.6); margin-top: 0.15rem; }
        ...
        .imp-bar i { display: block; height: 100%; border-radius: 2px; transform: scaleX(0); transform-origin: left;
          background: linear-gradient(90deg, #ff7a2f, #ffb061); animation: growx 0.9s cubic-bezier(0.2,0.8,0.2,1) both; }

        /* Education */
        .edu { max-width: 460px; }
        .edu-cap { animation: pop 0.6s cubic-bezier(0.2,1.4,0.4,1) both; filter: drop-shadow(0 0 10px rgba(255,122,47,0.5)); }
        .edu-deg { font-size: 0.92rem; font-weight: 700; color: #fff; margin-top: 0.5rem; animation: rise 0.5s both 0.2s; }
        .edu-note { font-size: 0.64rem; color: rgba(244,239,233,0.6); margin-top: 0.2rem; animation: rise 0.5s both 0.32s; }
        .edu-certs { display: flex; flex-wrap: wrap; gap: 0.4rem; justify-content: center; margin-top: 0.6rem; }
        .edu-cert { font-size: 0.6rem; color: #ffb061; background: rgba(255,122,47,0.1); border: 1px solid rgba(255,122,47,0.3);
          border-radius: 9999px; padding: 0.28rem 0.6rem; animation: rise 0.5s both; }

        /* Let's talk */
        .talk { display: flex; gap: 1.4rem; }
        .talk-node { display: grid; place-items: center; gap: 0.45rem; color: #f4efe9; text-decoration: none; animation: pop 0.6s cubic-bezier(0.2,1.4,0.4,1) both; }
        .talk-node :global(svg) { width: 22px; height: 22px; padding: 14px; border-radius: 9999px;
          background: rgba(255,122,47,0.12); border: 1px solid rgba(255,122,47,0.4); color: #ffb061; box-sizing: content-box;
          transition: all 0.2s; }
        .talk-node:hover :global(svg) { background: rgba(255,122,47,0.25); transform: translateY(-3px); }
        .talk-node span { font-size: 0.62rem; color: rgba(244,239,233,0.7); }
